...
def display_click_data(clickData):
    ...
    return html.B(f"Flow: {point['y']:.2f} LPM, Effective Pressure: {point['text'].split(', ')[2].split(': ')[1]} Bar, Lugeon: {point['text'].split(', ')[4].split(': ')[1]}, Gauge Pressure: {point['text'].split(', ')[3].split(': ')[1]}, Volume: {point['text'].split(', ')[5].split(': ')[1]} L, Timestamp: {timestamp}")
